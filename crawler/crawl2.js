...
(async () => {
    ...
    for (const url of urls) {
        ...
        await page.waitForSelector('//*[@id="app"]/div[14]/div/div[1]/div/div/div[2]/div[4]/div[6]/a/div/div[2]/div[1]');

        // Extract data-item-id using the phone XPath
        const dataItemId = await page.evaluate(() => {
             const element = document.evaluate('//*[@id="app"]/div[14]/div/div[1]/div/div/div[2]/div[4]/div[6]/a/div/div[2]/div[1]', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
             return element ? element.innerText : 'Phone not found';
        });

        console.log(`URL: ${url}`);
        console.log(`Address: ${address}`);
        console.log(`Title: ${title}`);
        //console.log(`Phone: ${dataItemId}`);
    }
